...
TEST_F(QuantraComparisonTest, EquityOption_EuropeanAsianGeometric_NPVMatches) {
    const double spot = 100.0;
    const double strike = 100.0;
    const double vol = 0.20;
    const auto expiry = evaluationDate_ + QuantLib::Period(1, QuantLib::Years);
    ...
    std::vector<QuantLib::Date> fixingDates = {
        evaluationDate_ + QuantLib::Period(3, QuantLib::Months),
        evaluationDate_ + QuantLib::Period(6, QuantLib::Months),
        evaluationDate_ + QuantLib::Period(9, QuantLib::Months),
        expiry
    };

    auto payoff = std::make_shared<QuantLib::PlainVanillaPayoff>(QuantLib::Option::Call, strike);
    auto exercise = std::make_shared<QuantLib::EuropeanExercise>(expiry);
    auto qlOption = std::make_shared<QuantLib::DiscreteAveragingAsianOption>(
        QuantLib::Average::Geometric, 0.0, 0, fixingDates, payoff, exercise);
    auto qlSpot = QuantLib::Handle<QuantLib::Quote>(std::make_shared<QuantLib::SimpleQuote>(spot));
    auto qlVol = QuantLib::Handle<QuantLib::BlackVolTermStructure>(
        std::make_shared<QuantLib::BlackConstantVol>(
            evaluationDate_, QuantLib::TARGET(), vol, QuantLib::Actual365Fixed()));
    auto process = std::make_shared<QuantLib::BlackScholesMertonProcess>(
        qlSpot, discountHandle_, discountHandle_, qlVol);
    qlOption->setPricingEngine(
        std::make_shared<QuantLib::AnalyticDiscreteGeometricAveragePriceAsianEngine>(process));
    const double qlNpv = qlOption->NPV();
    ...
    auto f1 = b.CreateString("2025-04-15");
    auto f2 = b.CreateString("2025-07-15");
    auto f3 = b.CreateString("2025-10-15");
    auto f4 = b.CreateString("2026-01-15");
    auto fixingDatesFb = b.CreateVector(std::vector<flatbuffers::Offset<flatbuffers::String>>{f1, f2, f3, f4});
    quantra::EquityAsianFeatureBuilder afb(b);
    afb.add_average_type(quantra::enums::EquityAsianAverageType_Geometric);
    afb.add_fixing_dates(fixingDatesFb);
    afb.add_running_accumulator(0.0);
    afb.add_past_fixings(0);
    auto asianFb = afb.Finish();

    auto tradeId = b.CreateString("EQ_ASIAN_GEO_1Y");
    quantra::EquityOptionBuilder eob(b);
    eob.add_trade_id(tradeId);
    eob.add_underlying_id(undId);
    eob.add_quantity(1.0);
...
TEST_F(QuantraComparisonTest, EquityOption_EuropeanBarrier_NPVMatches) {
    const double spot = 100.0;
    const double strike = 100.0;
    const double vol = 0.20;
    const double barrierLevel = 120.0;
    const double rebate = 0.0;
    const auto expiry = evaluationDate_ + QuantLib::Period(1, QuantLib::Years);

    auto payoff = std::make_shared<QuantLib::PlainVanillaPayoff>(QuantLib::Option::Call, strike);
    auto exercise = std::make_shared<QuantLib::EuropeanExercise>(expiry);
    auto qlOption = std::make_shared<QuantLib::BarrierOption>(
        QuantLib::Barrier::UpOut, barrierLevel, rebate, payoff, exercise);
    auto qlSpot = QuantLib::Handle<QuantLib::Quote>(std::make_shared<QuantLib::SimpleQuote>(spot));
    auto qlVol = QuantLib::Handle<QuantLib::BlackVolTermStructure>(
        std::make_shared<QuantLib::BlackConstantVol>(
            evaluationDate_, QuantLib::TARGET(), vol, QuantLib::Actual365Fixed()));
    auto process = std::make_shared<QuantLib::BlackScholesMertonProcess>(
        qlSpot, discountHandle_, discountHandle_, qlVol);
    qlOption->setPricingEngine(std::make_shared<QuantLib::AnalyticBarrierEngine>(process));
    const double qlNpv = qlOption->NPV();
    ...
    quantra::EquityBarrierFeatureBuilder bfb(b);
    bfb.add_barrier_type(quantra::enums::EquityBarrierType_UpOut);
    bfb.add_level(barrierLevel);
    bfb.add_rebate(rebate);
    bfb.add_monitoring(quantra::enums::EquityBarrierMonitoring_Continuous);
    auto barrierFb = bfb.Finish();

    auto tradeId = b.CreateString("EQ_BARRIER_UP_OUT_1Y");
    quantra::EquityOptionBuilder eob(b);
    eob.add_trade_id(tradeId);
    eob.add_underlying_id(undId);
    eob.add_quantity(1.0);
