#ifndef QUANTRA_VANILLA_SWAP_PRICER_H
#define QUANTRA_VANILLA_SWAP_PRICER_H

/**
 * VanillaSwapPricer — pure QuantLib pricing core for vanilla swaps
...
struct VanillaSwapCmsPricerParams {
    ...
    /// Negative sentinel preserved: the pricer translates <=0 to QL's default.
...
struct VanillaSwapCmsLegData {
    ...
    /// Negative sentinel preserved: pricer falls back to swap-index spot_days.
    int fixingDays = -1;
    QuantLib::DayCounter dayCounter;
    QuantLib::BusinessDayConvention paymentConvention = QuantLib::ModifiedFollowing;
    double gear = 1.0;
...
public:
    VanillaSwapResult price(const VanillaSwapInputs& inputs,
...
#endif // QUANTRA_VANILLA_SWAP_PRICER_H
