#ifndef QUANTRA_BOOTSTRAP_CURVES_EVALUATOR_H
#define QUANTRA_BOOTSTRAP_CURVES_EVALUATOR_H

/**
 * BootstrapCurvesEvaluator — pure QuantLib core for the curve-sampling endpoint.
 *
 * The bootstrap itself happens transparently inside PricingRegistryBuilder
 * (shared infrastructure shared with every pricing product). This evaluator's
 ...
 * bootstrap_curves_mapper.{h,cpp}. The evaluator reads market data exclusively
...
/// calendar/BDC pair from `use_grid_calendar_for_advance` so the evaluator
...
/// `gridDates` and `pillarDates` are pre-resolved so the evaluator never needs
...
#endif // QUANTRA_BOOTSTRAP_CURVES_EVALUATOR_H
