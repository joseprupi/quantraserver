...
ZeroCouponInflationSwapPriceResult ZeroCouponInflationSwapPricingService::price(
    ...
    if (!tradePricing || !tradePricing->zero_coupon_inflation_swap()) {
        QUANTRA_ERROR("PriceZeroCouponInflationSwap entry requires zero_coupon_inflation_swap");
    }
    if (!tradePricing->discounting_curve()) {
        QUANTRA_ERROR("PriceZeroCouponInflationSwap entry requires discounting_curve");
    }
    if (!tradePricing->inflation_curve()) {
        QUANTRA_ERROR("PriceZeroCouponInflationSwap entry requires inflation_curve");
    }
    ...
    if (curveIt == reg.inflation.zeroInflationCurves.end() || !curveIt->second || curveIt->second->empty()) {
        QUANTRA_ERROR("Zero inflation curve not found: " + inflationCurveId);
    }
    ...
    if (metaIt->second.kind != enums::InflationCurveKind_ZeroInflation) {
        QUANTRA_ERROR("Inflation curve is not zero inflation: " + inflationCurveId);
    }
    ...
    if (!inflationIndex) {
        QUANTRA_ERROR("Inflation index is not zero inflation: " + indexId);
    }
