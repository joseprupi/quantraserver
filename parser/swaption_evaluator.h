#ifndef QUANTRA_SWAPTION_EVALUATOR_H
#define QUANTRA_SWAPTION_EVALUATOR_H

/**
 * SwaptionEvaluator — pure QuantLib pricing core for swaptions.
 *
 ...
 * swaption_mapper.{h,cpp}. The evaluator reads market data exclusively from the
 ...
 * re-bootstrap is upstream of the evaluator in the legacy code path (it consumes
 * raw FB tables for curves/quotes/indices). To keep the evaluator FB-free without
 ...
 * hands them to the evaluator as plain data on the inputs — no callable, no FB
 * indirection. The QL Swaption itself is rebuilt from the plain-domain
 * SwaptionInstrument carried on each trade, so the evaluator reconstructs the
...
/// lifted out of the FB request by the mapper. The evaluator reconstructs the
...
/// snapshot's curves are bootstrapped at the SAME evaluation date the evaluator
...
/// Plain SwaptionTrade — everything the evaluator needs to value one swaption,
...
///   - A plain SwaptionInstrument from which the evaluator reconstructs the QL
...
/// asks for diagnostics. The evaluator finalizes each unique SABR vol surface at
...
#endif // QUANTRA_SWAPTION_EVALUATOR_H
