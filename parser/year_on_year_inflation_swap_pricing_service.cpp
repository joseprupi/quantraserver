...
YearOnYearInflationSwapPriceResult YearOnYearInflationSwapPricingService::price(
    ...
    if (!tradePricing || !tradePricing->year_on_year_inflation_swap()) {
        QUANTRA_INVALID_ARGUMENT("PriceYearOnYearInflationSwap entry requires year_on_year_inflation_swap");
    }
    ...
    if (curveIt == reg.inflation.yoyInflationCurves.end() || !curveIt->second || curveIt->second->empty()) {
        QUANTRA_NOT_FOUND("YoY inflation curve not found: " + inflationCurveId);
    }
    ...
    if (metaIt->second.kind != enums::InflationCurveKind_YoYInflation) {
        QUANTRA_INVALID_ARGUMENT("Inflation curve is not YoY inflation: " + inflationCurveId);
    }
    ...
    if (!inflationIndex) {
        QUANTRA_INVALID_ARGUMENT("Inflation index is not YoY inflation: " + indexId);
    }
