#ifndef QUANTRA_SAMPLE_VOL_SURFACES_EVALUATOR_H
#define QUANTRA_SAMPLE_VOL_SURFACES_EVALUATOR_H

/**
 * SampleVolSurfacesEvaluator — pure QuantLib core for the volatility-surface
 * sampling endpoint.
 *
 ...
 *
 * Date-grid resolution happens HERE in the evaluator, not in the mapper, because
 ...
 * mirrors (SampleDateGridSpec) and leaves the date math to the evaluator, which
 ...
 * sample_vol_surfaces_mapper.{h,cpp}. The evaluator reads market data only through
...
/// wire enum so the evaluator converts to QuantLib at the exact point the legacy
...
struct SampleVolSurfacesQuery {
    ...
    /// hook when registry construction failed; the evaluator short-circuits such
...
#endif // QUANTRA_SAMPLE_VOL_SURFACES_EVALUATOR_H
