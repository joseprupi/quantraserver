#ifndef QUANTRA_OIS_SWAP_PRICER_H
#define QUANTRA_OIS_SWAP_PRICER_H

/**
 * OisSwapPricer — pure QuantLib pricing core for overnight-indexed swaps
 ...
 * ois_swap_mapper.{h,cpp}. The pricer reads market data exclusively from the
...
struct OisSwapOvernightLegData {
    ...
    /// Negative sentinel preserved: pricer translates <0 to QL's Null<Natural>().
    int lookbackDays = -1;
    int lockoutDays = 0;
...
class OisSwapPricer {
public:
    OisSwapResult price(const OisSwapInputs& inputs,
...
#endif // QUANTRA_OIS_SWAP_PRICER_H
