#ifndef QUANTRA_BOOTSTRAP_INFLATION_CURVES_EVALUATOR_H
#define QUANTRA_BOOTSTRAP_INFLATION_CURVES_EVALUATOR_H

/**
 * BootstrapInflationCurvesEvaluator — pure QuantLib core for the inflation
 * curve sampling endpoint.
 *
 * Bootstrapping itself happens transparently inside PricingRegistryBuilder
 * (shared infrastructure); this evaluator's only job is to sample each
 ...
 * bootstrap_inflation_curves_mapper.{h,cpp}. The evaluator reads market data
...
/// reference date (taken from the FB InflationCurveSpec) so the evaluator
...
struct BootstrapInflationCurvesQuery {
    ...
    /// run; the evaluator emits a per-curve error entry verbatim and skips
...
class BootstrapInflationCurvesEvaluator {
public:
    BootstrapInflationCurvesResult evaluate(
...
#endif // QUANTRA_BOOTSTRAP_INFLATION_CURVES_EVALUATOR_H
