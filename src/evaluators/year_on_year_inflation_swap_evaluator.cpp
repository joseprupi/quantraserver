...
YearOnYearInflationSwapPerSwap priceTrade(const YearOnYearInflationSwapTrade& trade,
    ...
    if (includeFlows) {
        ...
        for (const auto& cf : swap->fixedLeg()) {
            YearOnYearInflationSwapFlowPlain flow;
            if (extractFlow(cf, *discountCurve, ctx.asOf, flow)) {
                out.fixedFlows.push_back(std::move(flow));
            }
        }
        for (const auto& cf : swap->yoyLeg()) {
            YearOnYearInflationSwapFlowPlain flow;
            if (extractFlow(cf, *discountCurve, ctx.asOf, flow)) {
                out.yoyFlows.push_back(std::move(flow));
            }
        }
    }
    return out;
...
YearOnYearInflationSwapResult YearOnYearInflationSwapEvaluator::evaluate(
    const YearOnYearInflationSwapInputs& inputs,
    const PricingRegistry& reg,
    const PricingContext& ctx) const {

    YearOnYearInflationSwapResult result;
